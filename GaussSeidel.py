...
def IterasiGaussSeidel3(A11, A12, A13, B1, A21, A22, A23, B2, A31, A32, A33, B3):
    ...
    print(":::Menggunakan Lelaran Gauss-Seidel:::")
    ...
    while True:
        ...
        tabel_jacobi.add_row([m, X1old, X2old, X3old, X1errold, X2errold, X3errold])
        ...
        if (abs(X1errold) == 0 and abs(X2errold) == 0 and abs(X3errold) == 0) or (m == 200):
            print(tabel_jacobi)
            print(f'\n Hasil Iterasi Gauss-Seidel -->     X1 = {int(X1old)}, X2 = {int(X2old)}, X3 = {int(X3old)}')
...
def IterasiGaussSeidel4(A11, A12, A13, A14, B1, A21, A22, A23, A24, B2, A31, A32, A33, A34, B3, A41, A42, A43, A44, B4):
    tabel_jacobi = PrettyTable()
    ...
    while True:
        ...
        X2new = (B2 - (A21 * X1new) - (A23 * X3old) - (A24 * X4old)) / A22
        X3new = (B3 - (A31 * X1new) - (A32 * X2new) - (A34 * X4old)) / A33
        X4new = (B4 - (A41 * X1new) - (A42 * X2new) - (A43 * X3new)) / A44
        ...
        if (abs(X1errold) == 0 and abs(X2errold) == 0 and abs(X3errold) == 0 and abs(X4errold) == 0) or (m == 200):
            print(tabel_jacobi)
            print(f'\n Hasil Iterasi Gauss-Seidel -->     X1 = {int(X1old)}, X2 = {int(X2old)}, X3 = {int(X3old)}, X4 = {int(X4new)}')
            print(f' Didapatkan setelah {m}x iterasi')
            if m == 200:
                print(" (MAKSIMAL)")
            break
